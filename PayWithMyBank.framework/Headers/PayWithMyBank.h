...
/*!
    ...
    @param onExternalUrlHandler Called when the PayWithMyBank panel must open an external URL. If not handled an internal in app WebView willshow the external URL.The external URL is sent on the returnParameters entry key “url”.
*/
- (void) onExternalUrl:(PayWithMyBankCallback)onExternalUrl;

@end

@interface PayWithMyBankView : UIView <PayWithMyBank>
...
typedef PayWithMyBankView PayWithMyBankPanel;
