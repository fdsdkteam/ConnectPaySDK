 ...
 */
typedef NS_ENUM(NSInteger, RLTMXProfilingConnectionMethod)
{
    RLTMXProfilingConnectionMethodPost = 0,
    RLTMXProfilingConnectionMethodGet  = 1
};
 ...
 */
@protocol RLTMXProfilingConnectionsProtocol <NSObject>
 ...
 */
- (void)httpProfilingRequestWithUrl:(NSURL * _Nonnull)url method:(RLTMXProfilingConnectionMethod)method headers:(NSDictionary * _Nullable)headers postBody:(NSData * _Nullable)postData completionHandler:(void (^_Nullable)(NSData * _Nullable result, NSError * _Nullable error))completionHandler NS_SWIFT_NAME(httpProfilingRequest(url:method:headers:postData:completionHandler:));
